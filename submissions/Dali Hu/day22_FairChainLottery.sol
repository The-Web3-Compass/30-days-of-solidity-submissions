...
contract FairChainLottery is VRFConsumerBaseV2Plus {
    enum LOTTERY_STATE { OPEN, CLOSED, CALCULATING } // 枚举了彩票可能的三种状态
    LOTTERY_STATE public lotteryState;

    address payable[] public players; //可支付地址的数组
    address public recentWinner;
    uint256 public entryFee;

    // Chainlink VRF 配置
    uint256 public subscriptionId;
    bytes32 public keyHash; //每条链需要使用它专属的 keyHash
    ...
    function endLottery() external onlyOwner {
        ...
        VRFV2PlusClient.RandomWordsRequest memory req = VRFV2PlusClient.RandomWordsRequest({
            ...
            )//把“附加参数”包装成 VRF 请求可以理解的 bytes 格式（struct → library 函数 → bytes）
        });
    ...
    function fulfillRandomWords(uint256, uint256[] calldata randomWords) internal override {
        ...
        uint256 winnerIndex = randomWords[0] % players.length;//取余数做为冠军
