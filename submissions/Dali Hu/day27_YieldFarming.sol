...
import "@openzeppelin/contracts/utils/math/SafeCast.sol"; // 防止小数意外溢出或数据丢失

// 用于获取 ERC-20 元数据(小数位数)的接口
//在 ERC20 接口（IERC20）的基础上，额外增加新的功能
...
contract YieldFarming is ReentrancyGuard {
    ...
    IERC20 public stakingToken;//用户必须存入质押的代币（仅一种）
    IERC20 public rewardToken;//平台支付的奖励代币
    ...
    event RewardClaimed(address indexed user, uint256 amount);//领取待处理的奖励
    event EmergencyWithdraw(address indexed user, uint256 amount);//紧急取出他们的质押
    ...
    constructor(
        ...
        } catch (bytes memory) {//如果调用失败执行catch里的逻辑
            stakingTokenDecimals = 18; // 如果获取失败,默认为 18 位小数
        }
    }

    ///     质押代币以开始赚取奖励
    function stake(uint256 amount) external nonReentrant {//防止重入
    ...
    ///     取消质押代币并可选择领取奖励
    function unstake(uint256 amount) external nonReentrant {//防止重入
    ...
    function updateRewards(address user) internal {
        ...
        if (staker.stakedAmount > 0) {
            ...
            uint256 rewardMultiplier = 10 ** stakingTokenDecimals;//为了处理代币小数，保证奖励计算精确
