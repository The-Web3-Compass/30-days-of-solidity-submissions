...
contract SimpleStableCoin is ERC20, Ownable, ReentrancyGuard, AccessControl {
    ...
    event Minted(address indexed user, uint256 amount, uint256 callateralDeposited);
    ...
    function redeem(uint256 amount) external nonReentrant {
        ...
        uint256 collaterationToReturn = (stablecoinValueUSD * 100) / (collateralizationRatio * collateralPrice);
        uint256 adjustedCollateralToReturn = (collaterationToReturn * (10 ** collateralDecimals)) / (10 **priceFeed.decimals());
    ...
    function getRequiredCollateralForMint(uint256 amount) public view returns (uint256){
        ...
        uint256 requiredCollateralValueUSD = amount * (10 ** decimals());
        uint256 requiredCollateral = (requiredCollateralValueUSD * collateralizationRatio) / (100 * collateralPrice);
        uint256 adjustedRequiredCollateral = (requiredCollateral * (10 ** priceFeed.decimals()));
