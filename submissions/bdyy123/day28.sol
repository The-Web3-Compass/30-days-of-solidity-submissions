...
contract DecentralizedGovernance is ReentrancyGuard {
    ...
    event QuorunNotMet(uint256 id, uint256 votesTotal, uint256 quorumNeeded);
    ...
    event QuorumNotMet(uint256 proposalId, uint256 totalVotes, uint256 quorumNeeded);
    event ProposalDepositPaid(address proposer,uint256 proposalDepositAmount);
    ...
    function setQuorumPercentage(uint256 _quorumPercentage) external onlyAdmin {
        require(_quorumPercentage <= 100, "Quorum percentage must between 0 and 100");
    ...
    function vote(uint256 proposalId, bool support) external {
        ...
        require(block.timestamp < proposal.deadline, "Voting peroid over");
        require(governanceToken.balanceOf(msg.sender) > 0, "No gpvernance tokend");
    ...
    function finalizeProposal(uint256 proposalId) external {
        Proposal storage proposal = proposals[proposalId];
        require(block.timestamp >= proposal.deadline, "Voting peroid not yet over");
