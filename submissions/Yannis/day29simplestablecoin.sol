...
contract SimpleStablecoin is ERC20, Ownable, ReentrancyGuard, AccessControl {
    ...
    function mint(uint256 amount) external nonReentrant {
        ...
        uint256 requiredCollateral = (requiredCollateralValueUSD *
            collateralizationRatio) / (100 * collateralPrice);

        
        uint256 adjustedRequiredCollateral = (requiredCollateral *
            (10 ** collateralDecimals)) / (10 ** priceFeed.decimals());

        
        collateralToken.safeTransferFrom(
            msg.sender,
            address(this),
            adjustedRequiredCollateral
        );
    ...
    function redeem(uint256 amount) external nonReentrant {
        ...
        uint256 collateralToReturn = (stablecoinValueUSD * 100) /
            (collateralizationRatio * collateralPrice);

        
        uint256 adjustedCollateralToReturn = (collateralToReturn *
            (10 ** collateralDecimals)) / (10 ** priceFeed.decimals());
    ...
    function getRequiredCollateralForMint(
        uint256 amount
    ...
    function getCollateralForRedeem(
        uint256 amount
    ) public view returns (uint256) {
        if (amount == 0) return 0;
