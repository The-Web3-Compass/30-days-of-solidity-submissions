...
contract PollStation{
    ...
    mapping(string => uint256) public voteCount; // mapping to store votes for each candidate

    function addCandidates(string memory _candidateNames)public{
        candidateNames.push(_candidateNames);
        voteCount[_candidateNames] = 0; // initialize vote count to 0 for new candidate
    }
