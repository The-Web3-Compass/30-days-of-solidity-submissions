...
// Design a simple tracker to track users' workout history
contract SimpleFitnessTracker{
    address public owner;

    // Use struct to record a list of information
    // User profile struct
    struct UserProfile{
        string name;
        uint256 weight;
    ...
    function updateWeight(uint256 _newWeight) public onlyRegistered{
        ...
        // Use UserProfile struct to define "storage" state variables.
    ...
    function logWorkout(string memory _activityType,uint256 _duration,uint256 _distance) public onlyRegistered{
        ...
        WorkoutActivity memory newWorkout=WorkoutActivity({
            activityType:_activityType,duration:_duration,distance:_distance,timestamp:block.timestamp
        });
