...
contract DecentralizedGovernance is ReentrancyGuard {
    ...
    function setQuorumPercentage(uint256 _quorumPercentage) external onlyAdmin {
        require(_quorumPercentage > 0, "Invalid percentage.");
    ...
    function createProposal(string calldata _description, address[] calldata _targets, bytes[] calldata _calldatas) 
        ...
        Proposal memory proposal = Proposal(nextProposalId,
                                    _description,
    ...
    function vote(uint256 proposalId, bool support) external {
        ...
        require(governanceToken.balanceOf(msg.sender) > 0, "Insufficient governance tokens");
    ...
    function finalizeProposal(uint256 proposalId) external {
        require(proposalId > 0 && proposalId < nextProposalId, "Invalid proposal Id.");
        Proposal memory proposal = proposals[proposalId];
        require(block.timestamp > proposal.deadline, "Voting has not ended.");
