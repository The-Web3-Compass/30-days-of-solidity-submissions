...
contract YieldFarming is ReentrancyGuard {
    ...
    function unstake(uint256 amount) external nonReentrant {
        require(amount > 0, "Invalid amount.");
        require(amount <= stakers[msg.sender].stakedAmount, "Insufficient balance.");
        updateRewards(msg.sender);
    ...
    function claimRewards() external nonReentrant {
        ...
        uint256 rewardAmount = stakers[msg.sender].rewardDebt;
        require(rewardAmount > 0, "No rewards to claim.");
        require(stakingToken.balanceOf(address(this)) >= rewardAmount, "Insufficient rewards.");
    ...
    function refillRewards(uint256 amount) external onlyOwner {
        ...
        emit RewardRefilled(owner, amount);
    }
