...
contract DecentralizedGovernance is ReentrancyGuard {
    ...
    event ProposalCreated(uint256 indexed id, string description, address indexed proposer, uint256 depositAmount);
    event Voted(uint256 indexed proposalId, address indexed voter, bool support, uint256 weight);
    event ProposalExecuted(uint256 indexed id, bool passed);
    event QuorumNotMet(uint256 indexed id, uint256 votesTotal, uint256 quorumNeeded);
    ...
    event ProposalTimelockStarted(uint256 indexed proposalId, uint256 executionTime);
    event QuorumPercentageSet(uint256 newPercentage);
    event ProposalDepositAmountSet(uint256 newAmount);
    ...
    function setProposalDepositAmount(uint256 _proposalDepositAmount) external onlyAdmin {
        ...
        emit ProposalDepositAmountSet(_proposalDepositAmount);
    }
    ...
    function createProposal(
        ...
        for (uint256 i = 0; i < _targets.length; i++) {
            ...
            require(_calldatas[i].length > 0, "Empty calldata");
        }

        governanceToken.safeTransferFrom(msg.sender, address(this), proposalDepositAmount);
    ...
    function finalizeProposal(uint256 proposalId) external validProposal(proposalId) {
        ...
        if (totalVotes >= quorumNeeded && proposal.votesFor > proposal.votesAgainst) {
            ...
            if (totalVotes < quorumNeeded) {
                governanceToken.safeTransfer(proposal.proposer, proposalDepositAmount);
    ...
    function executeProposal(uint256 proposalId) external nonReentrant validProposal(proposalId) {
        ...
        if (passed) {
            for (uint256 i = 0; i < proposal.executionTargets.length; i++) {
                ...
                require(success, string(abi.encodePacked("Call failed at index ", _toString(i), ": ", _extractRevertReason(returnData))));
            }
    ...
    function _extractRevertReason(bytes memory revertData) internal pure returns (string memory) {
        ...
        return abi.decode(revertReasonBytes, (string));
    }

    function emergencyTokenRecovery(address token, address to, uint256 amount) external nonReentrant onlyAdmin {
