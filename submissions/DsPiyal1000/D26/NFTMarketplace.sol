...
contract NFTMarketplace is ReentrancyGuard {
    ...
    function listNFT(
        ...
        require(nftAddress.code.length > 0, "NFT contract not deployed");
        require(
            royaltyReceiver != address(0) || royaltyPercent == 0,
        ...
        require(royaltyPercent + marketplaceFeePercent <= 1000, "Combined fees exceed 10%");
    ...
    function buyNFT(address nftAddress, uint256 tokenId) external payable nonReentrant {
        ...
        require(
            nft.getApproved(item.tokenId) == address(this) || nft.isApprovedForAll(item.seller, address(this)),
            "NFT approval revoked"
        );

        uint256 feeAmount = (msg.value * marketplaceFeePercent) / 1000;
        uint256 royaltyAmount = (msg.value * item.royaltyPercent) / 1000;
        uint256 sellerAmount = msg.value - feeAmount - royaltyAmount;

        if (item.royaltyReceiver == item.seller) {
