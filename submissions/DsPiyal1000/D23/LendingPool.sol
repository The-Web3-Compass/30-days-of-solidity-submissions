...
contract SimpleLending is ReentrancyGuard {
    ...
    uint256 public constant interestRateBasisPoints = 500;
    uint256 public constant collateralFactorBasisPoints = 7500;
    ...
    event MaxBorrowLimitUpdated(uint256 oldLimit, uint256 newLimit);
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        uint256 availableLiquidity = address(this).balance - totalBorrows;
    ...
    function getMaxBorrowAmount(address user) external view returns(uint256) {
        ...
        uint256 remainingUserLimit = currentDebt >= maxBorrowLimitPerUser ? 0 : maxBorrowLimitPerUser - currentDebt;
        uint256 remainingCollateralLimit = currentDebt >= collateralBasedLimit ? 0 : collateralBasedLimit - currentDebt;
    ...
    function setMaxBorrowLimit(uint256 newLimit) external onlyOwner {
        uint256 oldLimit = maxBorrowLimitPerUser;
        maxBorrowLimitPerUser = newLimit;
        emit MaxBorrowLimitUpdated(oldLimit, newLimit);
    }
}