...
contract MiniDexFactory is Ownable{
    event PairCreadted(address indexed tokenA, address indexed tokenB, address pairAddress, uint);
    ...
    function createPair(address _tokenA, address _tokenB) external onlyOwner returns (address pair){
        ...
        (address token0, address token1) = _tokenA < _tokenB ? (_tokenA, _tokenB) : (_tokenB, _tokenB);
        pair = address(new MiniDexPair(token0, token1));
        getPair[token0][token1] = pair;
        getPair[token1][token0] = pair;

        allPairs.push(pair);
        emit PairCreadted(token0, token1, pair, allPairs.length -1);
    }
