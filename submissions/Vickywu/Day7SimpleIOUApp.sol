...
contract SimpleIOU{
    address public owner;  //定义管理员
    
    // Track registered friends
    mapping(address => bool) public registeredFriends;  //快速检查是否允许某人使用合约
    address[] public friendList;
    
    // Track balances
    mapping(address => uint256) public balances;  //这个映射只是跟踪谁拥有多少
    ...
    function addFriend(address _friend) public onlyOwner {
        require(_friend != address(0), "Invalid address");   //检查新地址是否不是0x0
        require(!registeredFriends[_friend], "Friend already registered");   //确保该人尚未被添加
        
        registeredFriends[_friend] = true;
        friendList.push(_friend);  //添加到 registeredFriends 和friendList中
    }
