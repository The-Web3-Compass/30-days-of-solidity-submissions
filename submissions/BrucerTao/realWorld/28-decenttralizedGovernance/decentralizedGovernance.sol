...
contract DecentralizedGovernance is ReentrancyGuard {
    ...
    function setProposalDespositAmount(uint256 _proposalDespoitAmount) external onlyAdmin{
        proposalDepositAmount = _proposalDespoitAmount;
    }
    ...
    function createProposal(
        ...
        require(governanceToken.blanceOf(msg.sender) >= proposalDepositAmount, "Insufficient tokens for deposit");
        ...
        proposals[nextProposalId] = Proposal({
            ...
            dealine: block.timestamp + votingDuration,
            votesFor: 0,
            votesAgainst: 0,
            executed: false,
            proposer: msg.sender,
            exectutionData: _calldatas,
    ...
    function vote(uint256 proposalId, bool support) external {
        ...
        require(block.timestamp < proposal.dealine, "voting period has ended");
    ...
    function finalizeProposal(uint256 proposalId) external {
        Proposal storage proposal = proposals[proposalId];
        require(block.timestamp >= proposal.dealine, "voting period not ended");
    ...
    function executeProposal(uint256 proposalId) external nonReentrant {
        Proposal storage proposal = proposals[proposalId];
        require(prposal.executionTime > 0 && block.timestamp >= proposal.executionTime, "timelock not yet expired");
        require(!proposal.executed, "proposal already executed");
        ...
        if(passed) {
            for (uint256 i = 0; i < proposal.executionTargets.length; i++) {
                (bool success, bytes memory returnData) = proposal.executionTargets[i].call(proposal.exectutionData[i]);
