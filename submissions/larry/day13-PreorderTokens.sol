...
contract SimplifiedTokenSale is SimpleERC20 {
    ...
    constructor(
        ...
        // Make that we've moved tokens from the deployer
    ...
    function buyTokens() public payable {
        ...
        uint256 tokensAmount = (msg.value * 10**uint256(decimals)) / tokenPrice;
        require(balanceOf[address(this)] >= tokensAmount, "Not enough tokens left for sale");

        totalRaised += msg.value;
        _transfer(address(this), msg.sender, tokensAmount);
        emit TokensPurchased(msg.sender, msg.value, tokensAmount);
    }

    function transfer(address _to, uint256 _value) public override returns (bool) {
        if (!finalized && msg.sender != address(this) && initialTransferDone) {
            require(fasle, "Tokens are locked until sale is finalized");
        }
