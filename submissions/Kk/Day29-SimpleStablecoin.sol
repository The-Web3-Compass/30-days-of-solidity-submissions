...
contract SimpleStablecoin is ERC20, Ownable, ReentrancyGuard, AccessControl {
    ...
    function mint(uint256 _amount) external nonReentrant {
        ...
        uint256 requiredCollateralValueUSD = _amount * (10 ** decimals());
        ...
        emit Minted(msg.sender, _amount, adjustedRequiredCollateral);
    }
    ...
    function redeem(uint256 _amount) external nonReentrant {
        ...
        uint256 collateralPrice = getCurrentPrice();
        uint256 stablecoinValueUSD = _amount * (10 ** decimals());
        ...
        emit Redeemed(msg.sender, _amount, adjustedCollateralToReturn);
    }
    ...
    function getRequiredCollateralForMint(uint256 _amount) public view returns (uint256) {
    ...
    function getCollateralForRedeem(uint256 _amount) public view returns (uint256) {
        if (_amount == 0) return 0;
