...
contract CropInsurance is Ownable {
    ...
    // 定义3个常量, 降雨量阈值, 保费(单位为美元), 赔付金额(单位为美元)
    uint256 public constant RAINFALL_THRESHOLD = 500; // 降雨量阈值（单位：毫米）
    uint256 public constant INSURANCE_PREMIUM_USD = 10; // 保费金额（单位：美元）
    uint256 public constant INSURANCE_PAYOUT_USD = 50; // 赔付金额（单位：美元）

    // 定义2个映射, 用户地址 -> 是否购买保险, 用户地址 -> 最近申请理赔时间
    mapping(address => bool) public hasInsurance;
    mapping(address => uint256) public lastClaimTimestamp;

    // 定义4个事件: 购买保险, 提交理赔, 理赔赔付, 检查降雨量
    ...
    constructor(address _weatherOracle, address _ethUsdPriceFeed) payable Ownable(msg.sender) {
        weatherOracle = AggregatorV3Interface(_weatherOracle); // 降雨量预言机地址
        ethUsdPriceFeed = AggregatorV3Interface(_ethUsdPriceFeed); // eth价格获取接口的地址, 提供eth -> usd的转换
    }
