...
interface IERC20Metadata is IERC20 {
    ...
    function symbol() external view returns (string memory); // 返回简易标识符
}

/// @title 收益耕作平台
// 质押代币以随时间赚取奖励,可选紧急提取和管理员补充
contract YieldFarming is ReentrancyGuard {
    using SafeCast for uint256;   // 对uint256 类型使用SafeCast的安全扩展

    IERC20 public stakingToken;   // 可质押的代币
    IERC20 public rewardToken;    // 奖励的代币

    uint256 public rewardRatePerSecond; // 每秒（在所有质押者之间）分配的奖励

    address public owner;               // 合约属主

    uint8 public stakingTokenDecimals;  // 存储质押代币的小数位数

    // 质押信息结构体, 包含质押的代币数量, 已获取但尚未领取的奖励数量, 上次更新奖励的时间
    struct StakerInfo {
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        // 更新质押金额之前先更新用户获取的奖励, 以免新的质押搞乱奖励的计算
