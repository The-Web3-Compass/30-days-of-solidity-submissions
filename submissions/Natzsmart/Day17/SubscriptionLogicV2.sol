...
contract SubscriptionLogicV2 is SubscriptionLogicV1 { // Inherit from V1
    ...
    /**
     * @notice Returns the pause start time for a user's subscription.
     * @param user The address of the user.
     * @return pauseStartTime The timestamp when the subscription was paused (0 if not paused).
     */
    function getPauseInfo(address user) external view returns (uint256 pauseStartTime) {
        pauseStartTime = userSubscriptions[user].pauseStartTime;
    }
