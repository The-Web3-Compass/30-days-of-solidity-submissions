...
// Works in REMIX (GitHub imports)
// Works in FOUNDRY (lib/openzeppelin-contracts)
...
contract YieldFarming is ReentrancyGuard {
    ...
    constructor(
        ...
        // Fetch decimals (try/catch supported in both Remix + Foundry)
        try IERC20Metadata(_stakingToken).decimals() returns (uint8 decs) {
            stakingTokenDecimals = decs;
        } catch {
    ...
    function unstake(uint256 amount) external nonReentrant {
        require(amount > 0, "Zero amount");
        require(stakers[msg.sender].stakedAmount >= amount, "Not enough stake");
    ...
    function pendingRewards(address user) external view returns (uint256) {
        ...
        if (s.stakedAmount > 0) {
            ...
            uint256 multiplier = 10 ** stakingTokenDecimals;
            pending += (timeDiff * rewardRatePerSecond * s.stakedAmount) / multiplier;
        }
    ...
    function _updateRewards(address user) internal {
        ...
        if (s.stakedAmount > 0) {
            uint256 timeDiff = block.timestamp - s.lastUpdate;
            uint256 multiplier = 10 ** stakingTokenDecimals;
            uint256 pending = (timeDiff * rewardRatePerSecond * s.stakedAmount) / multiplier;
